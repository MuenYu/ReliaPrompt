...
app.post('/api/config', (req, res) => {
  try {
    const { openai_api_key, bedrock_access_key_id, bedrock_secret_access_key, bedrock_region, grok_api_key } = req.body;
    
    if (openai_api_key !== undefined) setConfig('openai_api_key', openai_api_key);
    if (bedrock_access_key_id !== undefined) setConfig('bedrock_access_key_id', bedrock_access_key_id);
    if (bedrock_secret_access_key !== undefined) setConfig('bedrock_secret_access_key', bedrock_secret_access_key);
    if (bedrock_region !== undefined) setConfig('bedrock_region', bedrock_region || 'us-east-1');
...
app.get('/api/test/status/:jobId', (req, res) => {
  try {
    ...
    res.json({
      ...
      progress: job.total_tests > 0 ? Math.round((job.completed_tests / job.total_tests) * 100) : 0,
