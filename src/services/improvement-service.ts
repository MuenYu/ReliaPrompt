...
async function runImprovement(
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    ...
    for (const improvement of improvements) {
      ...
      try {
        const result = await runTestsForPromptContent(improvement.prompt, testCases, clients);
    ...
    if (bestImprovement) {
      log(`Best improvement this iteration: ${bestImprovement.llm} with score ${bestImprovement.score}%`);
